...
export function DeathDateNudgeCard({ nudge, onSavedAndDone, onSkip, onSelectPerson }: Props) {
  ...
  const templates = useMemo(() => {
    const raw = tMessages.raw('deathDate');
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await personsService.update(nudge.person_id, { death_date: dateToIsoDay(date) });
      setSavedFlash(true);
      setTimeout(() => {
        setSlidingOut(true);
